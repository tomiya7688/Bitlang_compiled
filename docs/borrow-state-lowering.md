...
This document defines how borrow-state semantics arriving from Bitlang Preprocessed are handled when producing **Bitlang compiled**.
...
Before Bitlang compiled is emitted, the incoming Bitlang Preprocessed program has already resolved applicable borrow state explicitly as one of:
...
Bitlang compiled is not required to retain these high-level property names verbatim.
...
A program must not be lowered as valid Bitlang compiled when analysis proves any of the following:
...
Suspicious cases that cannot be proven invalid may have been diagnosed earlier as warnings. Bitlang compiled itself should receive a deterministic resolved program rather than re-run source-level heuristic inference.
